...
def setup_custom_fields(args=None):
	custom_fields = {
		...
		"Item": [
			...
			dict(
				fieldname="marketplace_item_group",
				label="Marketplace Item Group",
				fieldtype="Data",
		...
		"Sales Order": [
			...
			dict(
				...
				fieldtype="Link",
				options="Shopify Settings",
				insert_after="sb_shopify",
				read_only=1,
				print_hide=1,
			),
			dict(
			...
			dict(fieldname="cb_shopify", fieldtype="Column Break", insert_after="shopify_order_number"),
			dict(
				fieldname="shopify_order_id",
				label="Shopify Order ID",
				fieldtype="Data",
				insert_after="cb_shopify",
				read_only=1,
				print_hide=1,
				translatable=0,
			),
		...
		custom_fields.update(
			{
				"Shipstation Store": [
					...
					dict(
						fieldname="is_shopify_store",
						label="Is Shopify Store",
						fieldtype="Check",
						insert_after="sb_shopify",
						read_only=1,
						print_hide=1,
					...
					dict(
						...
						fieldtype="Link",
						options="Shopify Settings",
						insert_after="is_shopify_store",
						depends_on="eval:doc.is_shopify_store",
						print_hide=1,
					),
				]
			}
